...
def Create_args():
    ...
    parser.add_argument('--model_path', type=str,
                        default='/public/home/zzuegr01/lhf/Experimeat_02/GSM/model/acc_82.h5')
...
if __name__ == '__main__':
    ...
    probability_seg_= multi_gpu_model.predict(test_data)
        # probability_seg = model.predict(test_data)
    pre, rec,iou = acc.acc_2D_batch(probability_seg_, test_label)
    f1 = acc.acc_f1(pre, rec)
    print(args.model_path)
    
    print("test", pre, rec, f1,iou)
    acc_logger.output([args.model_path])
    acc_logger.output(["test", pre, rec, f1,iou])
    # exit()
