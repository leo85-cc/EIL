...
#拼接影像
def merge_map_segmentation(seg,name_list,thr=0.5,class_num=2):
    seg_inverse = []
    images=[];names=[]
    for i in range(0, len(seg)):
        ...
        if class_num==2:
            inverse =auxiliary_1C.convert_labels1C_inverse(seg[i],thr)
        elif class_num==3:
    ...
    while True:
        ...
        if args.map_channel==1:
            o_patch = np.fromstring(
                o_val, dtype=np.float16).reshape((o_side, o_side, args.sat_channel))  # 出错
